(() => {
  ...
  function initialize() {
    ...
    item = new Item(ctx, 0, 0, 32, 32, "./image/heart.png");
    item.setGirl(girl);
    ...
    boss = new Boss(ctx, 0, 0, 128, 128, "./image/green.png");
    ...
    for(i = 0; i < ENEMY_LARGE_MAX_COUNT; ++i) {
      ...
      enemyArray[ENEMY_RED_MAX_COUNT + ENEMY_BLUE_MAX_COUNT + i].setItem(item);
    }
    ...
    for(i = 0; i < SHOT_MAX_COUNT; ++i) {
      ...
      singleShotArray[i * 6] = new Shot( ctx, 0, 0, 32, 32, "./image/fire.png");
      singleShotArray[i * 6 + 1] = new Shot( ctx, 0, 0, 32, 32, "./image/fire.png");
      singleShotArray[i * 6 + 2]= new Shot( ctx, 0, 0, 32, 32, "./image/fire.png");
      singleShotArray[i * 6 + 3]= new Shot( ctx, 0, 0, 32, 32, "./image/fire.png");
      singleShotArray[i * 6 + 4]= new Shot( ctx, 0, 0, 32, 32, "./image/fire.png");
      singleShotArray[i * 6 + 5]= new Shot( ctx, 0, 0, 32, 32, "./image/fire.png");
      girl.setShotArray(shotArray, singleShotArray);
    }
    for(i = 0; i < SHOT_MAX_COUNT; ++i) {
      shotArray[i].setTargets(concatEnemyArray);
      singleShotArray[i * 6].setTargets(concatEnemyArray);
      singleShotArray[i * 6 + 1].setTargets(concatEnemyArray);
      singleShotArray[i * 6 + 2].setTargets(concatEnemyArray);
      singleShotArray[i * 6 + 3].setTargets(concatEnemyArray);
      singleShotArray[i * 6 + 4].setTargets(concatEnemyArray);
      singleShotArray[i * 6 + 5].setTargets(concatEnemyArray);

      shotArray[i].setExplosions(explosionArray);
      singleShotArray[i * 6].setExplosions(explosionArray);
      singleShotArray[i * 6 + 1].setExplosions(explosionArray);
      singleShotArray[i * 6 + 2].setExplosions(explosionArray);
      singleShotArray[i * 6 + 3].setExplosions(explosionArray);
      singleShotArray[i * 6 + 4].setExplosions(explosionArray);
      singleShotArray[i * 6 + 5].setExplosions(explosionArray);
    }
  ...
  function sceneSetting() {
    ...
    scene.add("block_default_type", (time) => {
      if(scene.frame % 30 === 0) {
    ...
    scene.add("block_default_type2", (time) => {
      if(scene.frame % 30 === 0) {
    ...
    scene.add("block_wave_move_type", (time) => {
      if(scene.frame % 50 === 0) {
    ...
    scene.add('block_large_type', (time) => {
      if(scene.frame === 100){
    ...
    scene.add('block_large_type2', (time) => {
      if(scene.frame === 100){
          let i = ENEMY_RED_MAX_COUNT + ENEMY_BLUE_MAX_COUNT + ENEMY_LARGE_MAX_COUNT;
      ...
      if(scene.frame === 0) {
        for(let i = 0; i < 20; ++i) {
          ...
          if(eL.life <= 0 && eR.life <= 0) {
            eL.set(eL.width / 2, eL.height * i + eL.height / 2, 100, "rush");
            eR.set(CANVAS_WIDTH - eR.width / 2, eR.height * i +eR.height / 2, 100, "rush");
          }
        }
      }
    ...
    scene.add("boss_coming", (time) => {
      ...
      if(boss.life > 5000 && boss.life <= 10000) {
        if(boss.mode === "rainy") {
          boss.posFlg = false;
          item.set(boss.position.x, boss.position.y);
        }
        ...
        if(boss.escapeStartTime + 10000 > Date.now()) {
          ...
          if(boss.callRush === true) {
            for(let i = 0; i < 20; ++i) {
              ...
              if(eL.life <= 0 && eR.life <= 0) {
                eL.set(eL.width / 2, eL.height * i + eL.height / 2, 100, "rush");
                eL2.set(eL.width * 1.5, eL.height * i + eL.height / 2, 100, "rush");
                eR.set(CANVAS_WIDTH - eR.width / 2, eR.height * i +eR.height / 2, 100, "rush");
                eR2.set(CANVAS_WIDTH - eR.width * 1.5, eR.height * i +eR.height / 2, 100, "rush");
              }
            }
            boss.callRush = false;
          }
      ...
      if(boss.life <= 0){
        ...
        for(i = 0; i < ENEMY_LARGE_MAX_COUNT; ++i) {
          explosionArray[ENEMY_RED_MAX_COUNT + ENEMY_BLUE_MAX_COUNT + i].set(enemyArray[i].position.x, enemyArray[i].position.y);
        }
      }
    });
    ...
    scene.add("gameclear", (time) => {
      ...
      let y = (scene.frame * 2) % loopHeight;
